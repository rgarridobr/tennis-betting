...
function parseAtpDateRange(
  ...
  const year = yearMatch ? yearMatch[1] : displayYear
...
export async function runAtpSync() {
  ...
  try {
    ...
    for (const group of data.TournamentDates) {
      ...
      for (const atp of group.Tournaments) {
        ...
        const atpNameNormalized = normalizeString(atp.Name)
        ...
        const bracketSize = Math.pow(2, Math.ceil(Math.log2(atp.SglDrawSize || bestMatch?.draw_size || 32)))

        // Check if we already have a tournament for this year and API ID
        const existingByApiId = await sql`
          SELECT id, slug FROM tournaments WHERE api_id = ${atp.Id} AND year = ${year}
        `

        if (existingByApiId.length > 0) {
          await sql`
    ...
    await sql`
      ...
      VALUES ('last_atp_sync', CURRENT_TIMESTAMP::text)

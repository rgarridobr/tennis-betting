  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || t('sync.toastError'));
      }
    } catch {
      toast.error(t('sync.toastErrorAtp'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative flex flex-col gap-2 w-full sm:w-auto group">
      ...
      {!canSync && !loading && (
        <div className="sm:hidden text-xs text-muted-foreground text-center">
          <p className="font-semibold">{t('sync.availableIn', { time: timeRemaining })}</p>
          {lastSync && (
            <p className="text-[10px]">
              {t('sync.lastSync', {
                time: formatDistanceToNow(lastSync, { addSuffix: true, locale: dateLocale }),
              })}
            </p>
          )}
        </div>
      ...
      {!canSync && !loading && (
        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden sm:block z-50 opacity-0 group-hover:opacity-100 transition">
          <div className="bg-slate-900 text-white text-xs rounded-xl p-3 shadow-xl whitespace-nowrap">
            <p className="font-bold">{t('sync.availableIn', { time: timeRemaining })}</p>
            {lastSync && (

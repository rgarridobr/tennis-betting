            ...
            <div className="flex flex-col md:flex-row md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                  {isViewingOthers ? t('viewingBracket') : t('bracket')}
                </h2>
                {enrolled && !started && !isViewingOthers && (
                  <span className="text-sm text-emerald-600 font-bold">{t('enrolledCta')}</span>
                )}
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-blue-900 font-black text-lg leading-tight">
                  {t('viewingPredictionsOf')}{' '}
                  <span className="text-blue-600">{targetUserInfo?.name || t('userFallback')}</span>
                </p>
              </div>
            </div>
            <Link
        ...
        enrolled ? (
          <div className="text-center py-24 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm px-6">
            ...
            <div className="mb-6 inline-flex items-center gap-2 px-4 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-sm font-bold border border-emerald-100">
              ...
              {participants} {participants === 1 ? t('enrolledOne') : t('enrolledOther')} {t('inTournament')}
            </div>

            <p className="text-slate-500 text-lg font-medium max-w-3xl mx-auto mb-4">
              {t('predictionsWhenBracket')}
            </p>

            <p className="text-slate-500 text-lg font-medium max-w-3xl mx-auto mb-4">
              {t('bracketAfterQualifying')}
            </p>

            <p className="text-slate-500 text-base font-medium max-w-2xl mx-auto mb-2">{t('comeBackAround')}</p>
            <p className="text-slate-500 text-lg font-bold max-w-2xl mx-auto capitalize">
              {twoDaysBefore.toLocaleDateString(dateLocale, {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
                year: 'numeric',
          ...
          (tournament.status === 'upcoming' ||
            ...
            tournament.status === 'STANDBY' ||
            tournament.status === 'UPCOMING' ||
            matches.length === 0) ? null : isViewingOthers && targetPredictions.length === 0 ? (
          <div className="text-center py-24 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm px-6">
            <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight mb-4">
              {t('userNoPredictionTitle')}
            </h3>
            <p className="text-slate-500 text-lg font-medium max-w-2xl mx-auto">
              {t('userNoPredictionBody')}
            </p>
          </div>
        ) : (
          <TournamentBracket

    ...
    <>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
           ...
           <Button asChild size="lg" className="bg-emerald-600 text-white hover:bg-emerald-700 font-black rounded-2xl px-8 shadow-lg shadow-emerald-200">
        ...
        {tournaments.length === 0 ? (
          ...
          <div className="space-y-12">
            {[
              { title: 'Rascunhos para finalizar', list: draftTournaments, icon: ClipboardList, color: 'text-rose-500' },
              ...
              .map((group) => (
                <section key={group.title}>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <Badge variant="outline" className="ml-2 font-bold">{group.list.length}</Badge>
                  </div>
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {group.list.map((tournament) => (
                      ...
                      >
                        <div
                          className={`h-3 ${
                            tournament.status === 'draft' ? 'bg-rose-500' :
                            (tournament.status === 'active' || tournament.status === 'published') ? 'bg-emerald-500' :
                            tournament.status === 'upcoming' ? 'bg-amber-400' :
                            'bg-slate-300'
                          }`}
                        />
                        <CardContent className="p-8">
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              <h3 className="font-black text-slate-900 text-2xl tracking-tight leading-tight">{tournament.name}</h3>
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500 mb-8">
                            <span className="px-4 py-1.5 bg-slate-100 rounded-full">{surfaceEmojis[tournament.surface] || tournament.surface}</span>
                          ...
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-6 border-t border-slate-100">
                            ...
                            <Button variant="outline" size="lg" asChild className="flex-1 rounded-2xl font-black border-2 hover:bg-slate-50 transition-colors">
